...
use privacy::objects::{
    ControlledNote, ControlledValidationContext, EncChannelInfo, EncOutgoingChannelInfo,
    EncPrivateKey, EncSubchannelInfo, Note, OpenNoteScreeningPolicy,
};
...
///
/// Validation runs inside the private proven execution and returns a small public authorization
/// receipt. The receipt is included in the proven server actions, so it must never contain private
/// transition data. `identity_key` is zero for a plain invoke; otherwise it and
/// `private_auxiliary_data` carry the private computation input. `public_calldata` is also passed
/// to the apply callback.
///
/// Apply runs onchain after proof verification. Implementations must authenticate both stages with
/// `validate_controlled_validation_context` and `validate_controlled_apply_caller`, and must reject
/// stale authorization by checking a current application epoch, phase, nonce, or deadline encoded
...
pub trait IControlledNoteController<T> {
    ...
    fn privacy_apply_controlled_transition(
        ref self: T, public_authorization: Span<felt252>, public_calldata: Span<felt252>,
    );
}
...
pub trait IServer<T> {
    ...
    /// dedicated controlled apply selector, which returns no data.
