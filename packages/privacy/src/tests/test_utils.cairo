...
#[test]
#[should_panic(expected: 'INVALID_CONTROLLED_AUTH')]
fn test_deserialize_controlled_authorization_rejects_oversized_receipt() {
    let mut authorization: Array<felt252> = array![];
    for value in 0..17_u32 {
        authorization.append(value.into());
    }
    let mut return_data = array![];
    authorization.span().serialize(ref return_data);
    deserialize_controlled_authorization(return_data.span());
}
