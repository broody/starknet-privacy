...
use privacy::tests::mock_note_controller::MockNoteController::{
    AUTHORIZATION, CALLBACK_MARKER, CONTROLLER_DENIED, STALE_AUTHORIZATION, WRONG_PRIVATE_AMOUNT,
};
...
fn test_controlled_note_lifecycle_uses_private_validation_and_explicit_authorization() {
    ...
    for action in actions {
        if let ServerAction::ControlledInvoke(input) = *action {
            ...
            assert_eq!(input.public_authorization, [AUTHORIZATION, 0].span());
...
#[test]
fn test_controlled_note_rejects_stale_epoch_and_rolls_back() {
    let mut test: Test = Default::default();
    let mut user = test.new_user();
    let (token, use_source, output) = source_note(ref test, ref user);
    let controller = deploy_mock_note_controller(
    ...
    dispatcher.set_epoch(epoch: 1);
    let result = test.privacy.safe_apply_actions(:actions);
    assert_panic_with_felt_error(:result, expected_error: STALE_AUTHORIZATION);
    let nullifier = compute_controlled_note_nullifier(
        note_id: created.note_id, spend_key: SPEND_KEY,
    );
    assert!(!test.privacy.nullifier_exists(:nullifier));
...
#[test]
fn test_controlled_note_transaction_rejects_open_note_outputs() {
    let mut test: Test = Default::default();
    let mut user = test.new_user();
    let (token, use_source, _) = source_note(ref test, ref user);
    let controller = deploy_mock_note_controller(
    ...
    let open_output = user
        .new_open_note_with_generated_random(recipient: user, token_addr: token, index: 0);
    ...
    user
        .assert_actions_panic(
            [
                ...
                ClientAction::CreateOpenNote(open_output),
                ClientAction::InvokeExternal(
                    InvokeExternalInput {
                        contract_address: controller, calldata: [CALLBACK_MARKER].span(),
                    },
                ...
                .span(),
            expected_error: errors::CONTROLLED_OPEN_OUTPUT_UNSUPPORTED,
        );
}
