...
pub mod Privacy {
    ...
    use privacy::objects::{
        ControlledDeposit, ControlledNote, ControlledNoteOpening, ControlledTransition,
        ControlledValidationContext, ControlledWithdrawal, EncChannelInfo, EncOutgoingChannelInfo,
        EncPrivateKey, EncSubchannelInfo, Note, OpenNoteDeposit, OpenNoteScreeningPolicy,
    ...
    use privacy::utils::constants::{
        ...
        CONTROLLED_VALIDATE_SELECTOR, DEPOSITOR_VALIDATION_MAX_AGE, DEPOSITOR_VALIDATION_MAX_FUTURE,
    ...
    use privacy::utils::{
        ...
        decode_note_amount, derive_public_key, deserialize_controlled_authorization,
    ...
    pub(crate) impl ClientInternalImpl of ClientInternalTrait {
        ...
        fn validate_controlled_invoke(
            ...
            let private_auxiliary_data: Array<felt252> = array![];
            self
                ._validate_controlled_transition(
                    :controller,
                    :controlled_context,
                    identity_key: Zero::zero(),
                    private_auxiliary_data: private_auxiliary_data.span(),
                    public_calldata: calldata,
                )
        }
        ...
        fn validate_controlled_compute_and_invoke(
            ...
            self
                ._validate_controlled_transition(
                    ...
                    :controlled_context,
                    :identity_key,
                    private_auxiliary_data: compute_additional_data,
                    public_calldata: invoke_additional_data,
                )
        }

        fn _validate_controlled_transition(
            self: @ContractState,
            controller: ContractAddress,
            controlled_context: ControlledValidationContext,
            ...
            private_auxiliary_data.serialize(ref validation_calldata);
            public_calldata.serialize(ref validation_calldata);
            let return_data = call_contract_syscall(
                ...
                .unwrap_or_else(|panic_data| propagate_external_panic(panic_data.span()));
            let public_authorization = deserialize_controlled_authorization(return_data);
            array![
                ServerAction::ControlledInvoke(
                    ControlledInvokeInput {
                        controller, public_authorization, calldata: public_calldata,
                    },
                ),
            ]
        }
        ...
        fn use_controlled_note(
            ...
            array![
                ...
                ServerAction::EmitControlledNoteUsed(
                    events::ControlledNoteUsed { nullifier, controller: note.controller },
                ),
            ]
        }
                ...
                for action in client_actions {
                    match *action {
                        ...
                        ClientAction::CreateOpenNote(_) => {
                            assert(false, errors::CONTROLLED_OPEN_OUTPUT_UNSUPPORTED);
                        },
    ...
    pub impl ServerInternalImpl of ServerInternalTrait {
        ...
        fn _apply_actions(
            ...
            for action in actions {
                match *action {
                    ...
                    ServerAction::ControlledInvoke(input) => self._apply_controlled_invoke(:input),
        ...
        /// Applies the public receipt produced by proof-time controlled-note validation.
        fn _apply_controlled_invoke(ref self: ContractState, input: ControlledInvokeInput) {
            let ControlledInvokeInput { controller, public_authorization, calldata } = input;
            let mut effective_calldata = array![];
            public_authorization.serialize(ref effective_calldata);
