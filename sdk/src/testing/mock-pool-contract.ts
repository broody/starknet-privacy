...
export class MockPoolContract implements MockContract, PoolContractInterface {
    ...
    if (target !== undefined) {
      for (const action of actions) {
        switch (action.type) {
          ...
          case "CreateOpenNote":
            throw new Error("Controlled-note transactions do not support open-note outputs");
          case "CreateControlledNote":
  ...
  private invoke(
    ...
    if (controlledBatch.target !== undefined) {
      ...
      const authorization = this.contracts.call(
        ...
        [controlledBatch.context, 0n, [], calldata]
      );
  ...
  private computeAndInvoke(
    ...
    if (controlledBatch.target !== undefined) {
      ...
      const authorization = this.contracts.call(
        contractAddress,
        "privacy_validate_controlled_transition",
        [controlledBatch.context, identityKey, computeAdditionalData, invokeAdditionalData]
      );
  ...
  private controlledInvoke(
    ...
    publicAuthorization: unknown,
    calldata: bigint[]
  ): MockServerAction {
    const authorization = (
      Array.isArray(publicAuthorization) ? publicAuthorization : [publicAuthorization]
    ).map(toBigInt);
    assert(authorization.length <= 16, () => "Controlled-note authorization exceeds 16 felts");
    return {
